...
public OnGameModeInit()
{
	...
	print("| * Загружаем маппинг...          |==>.......| 15%  |");
	LoadMapping_GameStart();
	print("|\tГотово!                                     |");

	// Заспавним машины
	print("| * Загружаем машины...           |====>.....| 50%  |");
	AddSpawningCar_GameStart();
	print("|\tГотово!                                     |");

	print("| * Подключение к БД...           |=======>..| 75%  |");
...
public OnPlayerClickMap(playerid, Float:fX, Float:fY, Float:fZ)
{
	...
	return 1; // <------- Как стало
	// А почему? Потому-что если ты не админ и тыкнул на карту, то ничего не вернуло бы.
}
...
CMD:time(playerid, params[])
{
	...
	timeNow = date("%hh:%ii", gettime());
	GameTextForPlayer(playerid, timeNow, 3000, 1);
	timeNow = date("%dd.%mm.%yyyy %hh:%ii%ss", gettime());
	// Зачем создавать несколько переменных, когда можно попользоваться одной? ;)
